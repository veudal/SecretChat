...
namespace SecretChat
{
    ...
    public partial class Settings : Window
    {
        ...
        {
            var dialogResult = ModernWpf.MessageBox.Show("Möchtest Du Deinen Account wirklich löschen? Beim löschen Deines Accounts wird Deine Mac-Adresse, sowie Dein Benutzername entfernt und all Deine Einstellungen. Du musst nach dem Löschen Deines Accounts wieder einen neuen Account erstellen um Secret Chat® zu nutzen. Deine Vertifizierung wird ebenfalls zurückgesetzt. Wenn Du auf 'Ja' drückst wird Dein Account gelöscht und Secret Chat® beendet.", "ACCOUNT LÖSCHEN", MessageBoxButton.YesNo, MessageBoxImage.Exclamation);
            if (dialogResult == MessageBoxResult.Yes)
            {
                string macAndUser = GetMac() + Environment.UserName;
                string code = null;
                ...
                foreach (var u in storedUserNames)
                {
                    if (u.storedMac == macAndUser)
                    {
                        ...
                        usersController.DeleteStoredUser(oldName, macAndUser, code);
                        try
                        {
                            onlineUsersController.DeleteOnlineUser(oldName, macAndUser, code);
                        }
                        catch
                        {
                ...
                catch
                {
                    ...
                    {
                        ModernWpf.MessageBox.Show("Die gespeicherten Profilbilder konnten nicht gelöscht werden, diese können manuell über diesen Pfad entfernt werden: " + path + "ProfilePictures\\");
                    }
                }
                ModernWpf.MessageBox.Show("Dein Account wurde gelöscht, Secret Chat® wird nun beendet.", "Info", MessageBoxButton.OK, MessageBoxImage.Information);
                Environment.Exit(0);
            }
        }
        ...
        private void PictureButton_Click(object sender, RoutedEventArgs e)
        {
            if (PictureButton.Content as string == "Profilbild")
            {
                ...
                if (fileDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        {
                            code = System.IO.File.ReadAllText(path + "code.txt");
                            usersController.InsertUser(name, macAndUser, base64Text, "true", code);
                        }
                        else
                        {
                            ModernWpf.MessageBox.Show("Fehler; es wurde kein Zugangs-Code gefunden, starte Secret Chat neu und gebe deinen Zugangs-Code ein.", "Ungültiger Authentifizierungscode", MessageBoxButton.OK, MessageBoxImage.Error);
            ...
            else
            {
                ...
                {
                    code = System.IO.File.ReadAllText(path + "code.txt");
                    usersController.InsertUser(name, macAndUser, "", "false", code);
                }
                else
                {
                    ModernWpf.MessageBox.Show("Fehler; es wurde kein Zugangs-Code gefunden, starte Secret Chat neu und gebe deinen Zugangs-Code ein.", "Ungültiger Authentifizierungscode", MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        private void DefaultPicture_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dialogResult == MessageBoxResult.Yes)
            {
                ...
                if(vertification == "true")
                    usersController.InsertUser(name, macAndUser, "x", "true", code);
                else
                    usersController.InsertUser(name, macAndUser, "x", "null", code);
